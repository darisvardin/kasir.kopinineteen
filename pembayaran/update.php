...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf(
    "UPDATE pembayaran SET namapembayaran =%s, ketpembayaran=%s WHERE idpembayaran=%s",
    inj($koneksi, $_POST['namapembayaran'], "text"),
    inj($koneksi, $_POST['ketpembayaran'], "text"),
    inj($koneksi, $_POST['idpembayaran'], "int")
  );
...
$query = sprintf("SELECT  * FROM pembayaran WHERE idpembayaran = %s ORDER BY idpembayaran ASC", inj($koneksi, $id, "int"));
              ...
              <div class="card">
                <div class="card-header">
                  <div class="d-flex align-items-center">
                    <h4 class="card-title">Ubah Data Pembayaran</h4>
                  </div>
                </div>
                <form action="<?php echo $editFormAction; ?>" name="form1" method="POST">
                  <div class="card-body">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" name="namapembayaran" value="<?php echo htmlentities($row['namapembayaran'], ENT_COMPAT, ''); ?>" />
                    </div>
                    <div class="form-group">
                      <label>Keterangan Pembayaran</label>
                      <input type="text" class="form-control" name="ketpembayaran" value="<?php echo htmlentities($row['ketpembayaran'], ENT_COMPAT, ''); ?>" />
                    </div>
                    <input type="hidden" name="MM_update" value="form1">
                    <button type="submit" class="btn btn-success" id="displayNotif">Simpan</button>
                    <a href="read.php" class="btn btn-danger">Kembali</a>
                  </div>
                  <input type="hidden" name="idpembayaran" value="<?php echo htmlentities($row['idpembayaran'], ENT_COMPAT, ''); ?>">
                </form>
              </div>
            </div>
          </div>
